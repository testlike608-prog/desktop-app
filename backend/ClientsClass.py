...
try:
    ...
    camera_barcode = None  # type: ignore

try:
    import live_image
except Exception:
    live_image = None  # type: ignore
    ...
    def get_state_snapshot(self):
        ...
        with self._state_lock:
            return {
                ...
                "endpoints": {
                    "VisionClient_TRIG": f"{_cfg.get('vision_trig_ip')}:{_cfg.get('vision_trig_port')}",
                    "VisionClient_ID":   f"{_cfg.get('vision_id_ip')}:{_cfg.get('vision_id_port')}",
                    "cobotClient":       f"{_cfg.get('cobot_ip')}:{_cfg.get('cobot_port')}",
                    "triggerserver":     f"{_cfg.get('trigger_server_ip')}:{_cfg.get('trigger_server_port')}",
                },
            }
        ...
        with self._start_stop_lock:
            ...
            try:
                if camera_hub is None:
                    raise ImportError("camera_hub not available")
            ...
            if _scan_mode == "camera":
                if _hub_ok:
                    try:
                        if camera_barcode is None:
                            raise ImportError("camera_barcode not available")
            ...
            try:
                if live_image is None:
                    raise ImportError("live_image not available")
                live_image.start()
